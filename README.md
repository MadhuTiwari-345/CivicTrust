...
flowchart TD
    ...
    B -->|No| D[Start Onboarding]
    
    D --> E[Select Jurisdiction]
    E --> F[Age Group]
    F --> G[Registration Status]
    G --> H[Country-Specific Setup]
    H --> I[Location Input]
    I --> J[Civic Interests]
    J --> K[Google Auth + Profile Creation]
    
    C --> L[📊 Dashboard]
    K --> L
    
    L --> M[🗳️ Voter Journey Checklist]
    L --> N[🤖 MythBuster AI Chat]
    L --> O[📅 Election Timeline]
    L --> P[🗳️ Ballot Simulation]
    L --> Q[📈 Results Dashboard]
    L --> R[❓ Help Center]
    L --> S[🛡️ ID Verification]
    L --> T[⚙️ Settings]
    
    M -->|Complete Step| U[+50 Civic Points]
    S -->|Verify| V[✅ Verified Badge]
```
...
erDiagram
    ...
    TIMELINE {
        ...
        string date
        string description
        string type "deadline | voting | info"
        string level "federal | state | local"
    }
    
    USERS ||--o{ STEPS : "has many"
